...
class Loss(nn.Module):
    def __init__(self, area=False, boundary=True, contour_th=1.5, ratio=1):
    ...
    def forward(self, x, label):
        ...
        if self.area:
            ...
            loss += area_loss
        if self.boundary:
            prob_grad = F.tanh(self.gradlayer(x.float()))
            label_grad = torch.gt(self.gradlayer(label.float()), self.cth).float()
